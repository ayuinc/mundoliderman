...
class Wall {
  ...
  public function status()
  {
    ...
    if ($offset != -1) {
      ...
      $status_limit = isset($status_limit) ? $status_limit : 5;
      $limit = $status_limit;
    }
  ...
  public function comment_post()
  {
    ...
    try {
      ...
      if ($this->post_is_solved($post_id)) {
        throw new Exception("Can't comment post solved");
      }
  ...
  private function post_is_solved($post_id) {
    $data_where = array(
      'id' => $post_id
    );
    $query = $this->EE->db->select('solved')->from('wall_status')->where($data_where)->get();
    if ($query->num_rows() <= 0) {
      return true;
    }
